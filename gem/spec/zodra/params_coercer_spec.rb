  ...
  describe ".call" do
    ...
    it "coerces boolean true values" do
      %w[true 1 yes].each do |value|
        expect(described_class.call(value, :boolean)).to eq(true), "expected #{value.inspect} to coerce to true"
      end
    end

    it "coerces boolean false values" do
      %w[false 0 no].each do |value|
        expect(described_class.call(value, :boolean)).to eq(false), "expected #{value.inspect} to coerce to false"
      end
    end
    ...
    it "passes through actual boolean values" do
      expect(described_class.call(true, :boolean)).to eq(true)
      expect(described_class.call(false, :boolean)).to eq(false)
    end
    ...
    describe "array coercion" do
      it "coerces array elements" do
        expect(described_class.call(["1", "2", "3"], :array, of: :integer)).to eq([1, 2, 3])
      end

      it "returns coercion error if any element fails" do
        expect(described_class.call(["1", "abc", "3"], :array, of: :integer)).to eq(:coercion_error)
      end
